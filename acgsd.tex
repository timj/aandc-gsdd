...
\citep{1980SPIE..264..298G}. At the time the FITS standard, which then
lacked the capability to use binary tables, was not deemed an
efficient format to store complex mm/submm time-series and
spectral-line data from single-dish telescopes.
...
observations (continuum and spectral-line instrumentation) and a
specification of how the bytes would be represented on disk.
...
to the spectrum and continuum data, \texttt{C1SNO} the scan number,
...
\citep{UNIPOPS}, was started in early 1989 and first released to users in early
