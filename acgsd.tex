...
of the resultant file formats, provide an overview of the GSDD requirements and
...
In the late 1970s and early 1980s millimetre and submillimetre
...
IRAM, University of Arizona and JCMT observatories. The format, agreed
...
\textbf{C2FR} & Radial focus & \textbf{C4SY} & Source Y & \textbf{C8EF} & Forward spillover \& scattering efficieny \\
...
{\color{red}
Are these PDFL format? Does the 12-m still write in GSDD format?
VAX floating point?
}
...
telescopes used what looked like very similar data reduction system,
...
staff was needed to maintain and develop each version.  The NRAO was
also migrating to Unix-based (primarily Sun) computers, a change that
would require major modifications to POPS.  {\color{red} (Do we want to
  mention this was also when Betty and Lorrie Morgan left the NRAO,
  which we anticipated would severely reduce further development of
  independent versions of POPS?)}  The unified analysis system, UniPOPS
\citep{UNIPOPS} {\color{red} (can we get ASCL entry? Need source
  code.)}, was started early 1989 and first released to users early
1991.  Although the 300ft collapsed in 1988, the 140ft was
...
POPS.  The UniPOPS data structure is, thus, identical to the PDFL
export files, which was the 12-m version of GSDD.  Adopting 140\,ft
and 300\,ft data to use the SDD format of SDD {\color{red} (typo?)} was
relatively easy, good evidence that GSDD was indeed a rather versatile
and useful standard.  {\color{red} (SDD is not spelled out at first
  usage.)}
...
changed the name of the export format to SDD.  Other than a
modification that expanded the capabilities of the index section of
the NRAO SDD files, the SDD format adopted for UniPOPS
...
UniPOPS could export and import data in SDFITS, and SDD, as well as many
of the historical NRAO formats.  The NRAO found that very few users
went away with SDFITs format while most took home SDD files.  Since
users were installing UniPOPs on their home computers, they probably
found transporting SDD files as being a bit more convenient than
...
system. Furthermore, when SDFITs was released, few FITS readers at the
...
the raw data file format.  The two obvious options were available in the
...
Global Section Datafile System \citep[GSD;][]{mtin33}\footnote{This
...
IEEE format. Furthermore, computers were significantly more powerful
...
not realise that there was a standard and therefore there was no
impetus for people to continue to communicate as systems were
...
the 12m GSDD system left NRAO before the end of the
decade. Interviewing people from NRAO and JCMT following the
...
by \htmladdnormallinkfoot{\textsc{class}}{http://www.iram.fr/IRAMFR/GILDAS}
\citep[][\ascl{1305.010}]{2005sf2a.conf..721P}.

In summary, it is impossible for a standard to survive as a standard
...
success when the format is looked at independently. The JCMT GSD
...
Starlink Project \citep{1982QJRAS..23..485D} to publish the
...
solutions. Today, 30 years on, the Internet and the culture of open-source
...
but failed in its original goal of unifying single dish millimetre
